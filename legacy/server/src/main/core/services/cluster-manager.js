...
class ClusterManager extends EventEmitter {
  ...
  async init() {
    ...
    setTimeout(() => {
      this._initialSync().catch(e => dbg(`[cluster] ❌ Erreur synchro initiale: ${e.message}`))
    }, 2000)
  }
  ...
  _startHeartbeat() {
    ...
    this._heartbeatInterval = setInterval(async () => {
      try {
        ...
        await this._checkPeers().catch(e => dbg(`[cluster] ❌ Erreur CheckPeers: ${e.message}`))
        if (this.role === 'slave') {
          await this._checkMasterHealth().catch(e => dbg(`[cluster] ❌ Erreur CheckMasterHealth: ${e.message}`))
        }
      } catch (e) {
