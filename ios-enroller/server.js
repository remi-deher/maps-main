...
app.post('/api/transfer', async (req, res) => {
    ...
    try {
        ...
        // Essayer d'abord l'endpoint moderne du moteur Go /api/device/enroll
        let response;
        try {
            console.log(`[DEBUG] Tentative d'envoi vers ${baseUrl}/api/device/enroll...`);
            response = await axios.post(`${baseUrl}/api/device/enroll`, payload, { timeout: 4000 });
            res.json({ success: true, data: response.data });
        } catch (err) {
            // Si 404 ou refusé, tenter l'ancien endpoint /api/enroll
            if (err.response && err.response.status === 404) {
                console.log(`[DEBUG] Endpoint moderne non trouvé (404), tentative avec l'ancien endpoint /api/enroll...`);
                response = await axios.post(`${baseUrl}/api/enroll`, payload, { timeout: 4000 });
                res.json({ success: true, data: response.data });
            } else {
                throw err;
            }
